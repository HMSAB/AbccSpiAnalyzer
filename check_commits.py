...
#   * Cache file stores SHA of each commit which was tested for compilation during last execution.subprocess
#       This allows the tool to skip re-compilation of those commits and just report the last results.
...
Example:
❯ python .\check_commits.py -b "python ./build_analyzer.py" -c 50 -s "dev" -e "origin/master"
...
def _cleanup(git_repo, original_branch_name, temp_branch_name):
...
def _check_commits(git_repo, args):
